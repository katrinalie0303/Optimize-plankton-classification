...
def balance_dataset(base_dataset_path, output_path):
	...
	set_path = os.path.join(base_dataset_path, "test")
	output_set_path = os.path.join(output_path, "test")

	# The number of images per class is the total number of images in the set divided by the number of classes.
	target_num_images_per_class = 5000#sum([len(os.listdir(os.path.join(set_path, class_name))) for class_name in os.listdir(set_path)]) // len(os.listdir(set_path))
