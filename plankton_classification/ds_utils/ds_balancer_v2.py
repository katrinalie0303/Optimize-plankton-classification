...
def balance_dataset(base_dataset_path, output_path):
	"""
    Balances the dataset by creating augmented images for under-represented classes.
	...
	#target_num_images_per_class = sum([len(os.listdir(os.path.join(set_path, class_name))) 
    #target_num_images_per_class = 50

for class_name in os.listdir(set_path)]) // len(os.listdir(set_path))
	...
	for class_name in os.listdir(set_path):
		...
		# Increase number of images on under-represented classes.		
		if n < 50:
			create_augmented_images(output_class_path, 50, generator)
...
def parse_arguments():
    ...
    """
    parser = argparse.ArgumentParser(description="Balance a dataset by creating augmented images for under-represented classes. Note that this script was developed on a linux system and may not work on other operating systems.")
    parser.add_argument('--source', '-s', type=str, required=True, help='The path to the base dataset directory.')
    parser.add_argument('--destination', '-d', type=str, required=True, help='The path to the directory where the dataset will be stored.')
